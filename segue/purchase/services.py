...
class PurchaseService(object):
    ...
    def migrate_type(self, purchase_id, new_type):
        purchase = self.get_one(purchase_id, strict=True, check_owner=False)
        ...
        return self.get_one(purchase_id, strict=True, check_owner=False)
    ...
    def get_one(self, purchase_id, by=None, strict=False, check_owner=True):
        purchase = Purchase.query.get(purchase_id)
        if strict and not purchase: raise NoSuchPurchase()
        if not purchase: return None
        if not check_owner: return purchase
